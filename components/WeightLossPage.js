...
async function aiLookupFood(query, lang = 'ru') {
  const prompt = lang === 'en'
    ? `You are a nutrition expert. For the food item "${query}", return ONLY a JSON object (no markdown, no explanation) with these fields: name (string, in the original language), calories (number, kcal per 100g or per serving if specified), protein (number, grams), fat (number, grams), carbs (number, grams). Example: {"name":"Boiled egg","calories":78,"protein":6.3,"fat":5.0,"carbs":0.6}`
    : `Ты эксперт по питанию. Для блюда или продукта "${query}" верни ТОЛЬКО JSON объект (без markdown, без пояснений) с полями: name (строка, название), calories (число, ккал на 100г или на порцию если указано), protein (число, граммы), fat (число, граммы), carbs (число, граммы). Пример: {"name":"Варёное яйцо","calories":78,"protein":6.3,"fat":5.0,"carbs":0.6}`
