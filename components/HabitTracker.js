...
function scheduleLabel(habit, lang = 'ru') {
  ...
  if (days.length === 7) return lang === 'en' ? '📅 Every day' : '📅 Ежедневно'
  if (JSON.stringify([...days].sort()) === JSON.stringify([0,1,2,3,4])) return lang === 'en' ? '💼 Weekdays' : '💼 Будни'
  if (JSON.stringify([...days].sort()) === JSON.stringify([5,6])) return lang === 'en' ? '🛋 Weekend' : '🛋 Выходные'
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:6, marginBottom:8, flexWrap:'wrap' }}>
        {[
          { label: i18n.habits?.allDays || 'Все', days: ALL_DAYS },
          { label: i18n.habits?.weekdays || 'Будни', days: WORKDAYS },
          { label: i18n.habits?.weekend || 'Выходные', days: WEEKEND },
        ].map(p => {
  ...
  return (
    <div style={{
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12 }}>
        ...
        }} title={done ? i18n.habits?.markUndone || 'Отметить как невыполненное' : i18n.habits?.markDone || 'Отметить как выполненное'}>
          {done ? '✓' : ''}
        </button>
...
function CreateHabitForm({ t, i18n = {}, onClose, onCreate }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    if (!form.name.trim()) { setError(i18n.habits?.habitNamePh || 'Введите название привычки'); return }
    if (form.frequency === 'custom' && form.days.length === 0) {
      setError(i18n.habits?.chooseDay || 'Выберите хотя бы один день'); return
    }
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json(); setError(d.error || i18n.common?.error || 'Ошибка'); setLoading(false); return }
      const created = await res.json()
      onCreate(created); onClose()
    } catch { setError(i18n.common?.networkError || 'Ошибка сети'); setLoading(false) }
  }
  ...
  return (
    <div style={{
      ...
      <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:14 }}>
        ...
        {/* Name */}
        <div>
          <label style={labelStyle}>{i18n.habits?.habitName || 'НАЗВАНИЕ *'}</label>
        ...
        {/* Description */}
        <div>
          <label style={labelStyle}>{i18n.habits?.description || 'ОПИСАНИЕ (необязательно)'}</label>
        ...
        {/* Frequency selector */}
        <div>
          <label style={labelStyle}>{i18n.habits?.schedule || 'РАСПИСАНИЕ'}</label>
          <div style={{ display:'flex', gap:8, marginBottom: form.frequency==='custom' ? 10 : 0 }}>
            {[
              { val:'daily',  icon:'📅', label:i18n.habits?.everyday || 'Каждый день' },
              { val:'custom', icon:'📆', label:i18n.habits?.byDays || 'По дням'     },
            ].map(opt => {
        ...
        {/* Color */}
        <div>
          <label style={labelStyle}>{i18n.habits?.color || 'ЦВЕТ'}</label>
        ...
        <div style={{ display:'flex', gap:10 }}>
          ...
          <button type="submit" disabled={loading} style={{
            ...
            {loading ? '...' : i18n.common?.create || 'Создать'}
          </button>
        </div>
      </form>
    </div>
