...
const ACHIEVEMENTS = [
  // ── Первые шаги ──
  {
    ...
    icon: '🌱', titleRu: 'Первый росток', titleEn: 'First Sprout',
    descRu: 'Создай свою первую привычку', descEn: 'Create your first habit',
    check: ({ habits }) => habits.length >= 1,
    progress: ({ habits }) => ({ cur: Math.min(habits.length, 1), max: 1 }),
  },
  {
    id: 'first_done',    category: 'start',
    icon: '✅', titleRu: 'Первый шаг', titleEn: 'First Step',
    descRu: 'Выполни привычку первый раз', descEn: 'Complete a habit for the first time',
  ...
  // ── Выполнения ──
  {
    ...
    icon: '💪', titleRu: 'В ритме', titleEn: 'In the Zone',
    descRu: 'Выполни привычки 10 раз суммарно', descEn: 'Complete habits 10 times total',
    check: ({ totalDone }) => totalDone >= 10,
    progress: ({ totalDone }) => ({ cur: Math.min(totalDone, 10), max: 10 }),
  },
  {
    id: 'done_50',       category: 'count',
    icon: '🏃', titleRu: 'Марафонец', titleEn: 'Marathon Runner',
    descRu: 'Выполни привычки 50 раз суммарно', descEn: 'Complete habits 50 times total',
    check: ({ totalDone }) => totalDone >= 50,
    progress: ({ totalDone }) => ({ cur: Math.min(totalDone, 50), max: 50 }),
  },
  {
    id: 'done_100',      category: 'count',
    icon: '🏆', titleRu: 'Сотня', titleEn: 'The Hundred',
    descRu: 'Выполни привычки 100 раз суммарно', descEn: 'Complete habits 100 times total',
    check: ({ totalDone }) => totalDone >= 100,
    progress: ({ totalDone }) => ({ cur: Math.min(totalDone, 100), max: 100 }),
  },
  {
    id: 'done_365',      category: 'count',
    icon: '🌟', titleRu: 'Посвящённый', titleEn: 'Dedicated',
    descRu: 'Выполни привычки 365 раз суммарно', descEn: 'Complete habits 365 times total',
  ...
  // ── Коллекционер ──
  {
    ...
    icon: '🌿', titleRu: 'Коллекционер', titleEn: 'Collector',
    descRu: 'Создай 3 разные привычки', descEn: 'Create 3 different habits',
    check: ({ habits }) => habits.length >= 3,
    progress: ({ habits }) => ({ cur: Math.min(habits.length, 3), max: 3 }),
  },
  {
    id: 'habits_5',      category: 'variety',
    icon: '🌳', titleRu: 'Архитект жизни', titleEn: 'Life Architect',
    descRu: 'Создай 5 разных привычек', descEn: 'Create 5 different habits',
  ...
  // ── Перфекционизм ──
  {
    ...
    icon: '⭐', titleRu: 'Идеальный день', titleEn: 'Perfect Day',
    descRu: 'Выполни все привычки за один день', descEn: 'Complete all habits in one day',
  ...
  },
  {
    ...
    icon: '🌟', titleRu: 'Идеальная неделя', titleEn: 'Perfect Week',
    descRu: 'Идеальный день 7 раз подряд', descEn: 'Perfect day 7 times in a row',
  ...
  // ── Секретные ──
  {
    ...
    icon: '🦉', titleRu: '???', titleEn: '???',
    descRu: 'Секретное достижение', descEn: 'Secret achievement',
...
// ── Компонент ─────────────────────────────────────────────────────────────────
export default function AchievementsPage({ t, lang = 'ru', i18n = {} }) {
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%', background: t.bg }}>
      ...
      <header style={{
        ...
        <div style={{ display:'flex', alignItems:'baseline', justifyContent:'space-between', marginBottom:12 }}>
          <span style={{ fontSize:20, fontWeight:800, color: t.text }}>{i18n.achievements?.title || 'Достижения'}</span>
        ...
        <div style={{ display:'flex', gap:8, marginBottom:14, overflowX:'auto',
          ...
          {[
            { label: lang === 'en' ? 'Max streak' : 'Макс. серия', value: `${maxStreak}🔥`, color: '#F0A30A' },
            { label: lang === 'en' ? 'Completed' : 'Выполнено', value: totalDone, color: t.success },
            { label: lang === 'en' ? 'Habits' : 'Привычек', value: habits.length, color: '#8B5CF6' },
          ].map(s => (
