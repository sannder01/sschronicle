...
const PRIORITY_CONFIG = {
  high:   { labelRu: 'Высокий', labelEn: 'High',   color: '#FF4466', xp: 50 },
  medium: { labelRu: 'Средний', labelEn: 'Medium', color: '#F0A30A', xp: 25 },
  low:    { labelRu: 'Низкий',  labelEn: 'Low',    color: '#5CB85C', xp: 10 },
}
...
function getDaysLeftLabel(days, lang = 'ru', i18n = {}) {
  if (days === null) return ''
  if (days < 0) return `−${Math.abs(days)}${lang === 'en' ? 'd' : 'д'}`
  if (days === 0) return lang === 'en' ? 'today' : 'сегодня'
  if (days === 1) return i18n.tasks?.tomorrow || 'завтра'
  return `+${days}${lang === 'en' ? 'd' : 'д'}`
}
...
function getDefaultFolders(i18n) {
  return [
    { id: 'all',    name: i18n?.tasks?.allTasks    || 'Все задачи', emoji: '◈', color: '#888' },
    { id: 'today',  name: i18n?.tasks?.todayTasks  || 'Сегодня',    emoji: '◎', color: '#F0A30A' },
    { id: 'urgent', name: i18n?.tasks?.urgent      || 'Срочные',    emoji: '◉', color: '#FF4466' },
  ]
}
...
// ═══════════════════════════════════════════════════════════════════
const PAGE_ICONS = ['◈','🔥','⚔️','📅','👤','📝','🏆','🎯']
function getPages(i18n) {
  const n = i18n.nav
  return [
    { id: 0, label: n.tasks,        icon: '◈' },
    { id: 1, label: n.habits,       icon: '🔥' },
    { id: 2, label: n.character,    icon: '⚔️' },
    { id: 3, label: n.calendar,     icon: '📅' },
    { id: 4, label: n.profile,      icon: '👤' },
    { id: 5, label: n.notes,        icon: '📝' },
    { id: 6, label: n.achievements, icon: '🏆' },
    { id: 7, label: n.weightLoss,   icon: '🎯' },
  ]
}
...
export default function PlannerClient() {
  ...
  async function createTask(e) {
    e.preventDefault()
    if (!formData.title.trim()) { setFormError(lang === 'en' ? 'Enter task title' : 'Введи название задания'); return }
    setFormError('')
    ...
    } catch { setFormError(lang === 'en' ? 'Connection error' : 'Ошибка соединения') }
  }
  ...
  function onTouchEnd(e) {
    ...
    if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 50) {
      if (dx < 0 && activePage < PAGE_ICONS.length - 1) setActivePage(p => p + 1)
  ...
  const allFolders = [...getDefaultFolders(i18n), ...folders]
  const activeFolderData = allFolders.find(f => String(f.id) === String(activeFolder)) || getDefaultFolders(i18n)[0]
        ...
        <div className="pc-overlay" style={{ zIndex: 900 }} onClick={() => setDeleteConfirm(null)}>
          <div className="pc-modal" onClick={e => e.stopPropagation()} style={{ background: '#111', borderColor: `${t.danger}33` }}>
            ...
            <div className="pc-modal-actions">
              <button type="button" className="pc-btn-ghost" style={{ color: t.textSub, borderColor: t.cardBorder }} onClick={() => setDeleteConfirm(null)}>{i18n.common?.cancel || 'Отмена'}</button>
              <button type="button" className="pc-btn-danger" style={{ background: t.danger }} onClick={() => deleteTask(deleteConfirm.id)}>{i18n.common?.delete || 'Удалить'}</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="pc-modal" onClick={e => e.stopPropagation()} style={{ background: '#111', borderColor: t.cardBorder }}>
            ...
            <form onSubmit={saveFolder} style={{ display:'flex', flexDirection:'column', gap:10 }}>
              ...
              <div className="pc-modal-actions">
                ...
                <button type="button" className="pc-btn-ghost" style={{ color: t.textSub, borderColor: t.cardBorder }} onClick={() => setEditingFolder(null)}>{i18n.common?.cancel || 'Отмена'}</button>
                <button type="submit" className="pc-btn-primary" style={{ background: editFolderForm.color || '#fff', color: '#000' }}>{i18n.common?.save || 'Сохранить'}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="pc-overlay" style={{ zIndex: 900 }} onClick={e => e.target === e.currentTarget && setEditingTask(null)}>
          <div className="pc-modal" style={{ background: '#111', borderColor: t.cardBorder }}>
            ...
            <form onSubmit={saveEditTask} style={{ display:'flex', flexDirection:'column', gap:10 }}>
              <input className="pc-input pc-input-title" style={{ background: t.inputBg, borderColor: t.cardBorder, color: t.text }} placeholder={i18n.tasks?.taskName || "Название задания..."} value={editForm.title} onChange={e => setEditForm({...editForm, title: e.target.value})} autoFocus />
              <div className="pc-form-row">
                <div className="pc-form-field"><label style={{ color: t.textSub }}>{i18n.tasks?.dueDate || 'Дата'}</label><input type="date" className="pc-input" style={{ background: t.inputBg, borderColor: t.cardBorder, color: t.text, colorScheme:'dark' }} value={editForm.due_date} onChange={e => setEditForm({...editForm, due_date: e.target.value})} /></div>
                <div className="pc-form-field"><label style={{ color: t.textSub }}>{i18n.tasks?.dueTime || 'Время'}</label><input type="time" className="pc-input" style={{ background: t.inputBg, borderColor: t.cardBorder, color: t.text, colorScheme:'dark' }} value={editForm.due_time} onChange={e => setEditForm({...editForm, due_time: e.target.value})} /></div>
                <div className="pc-form-field"><label style={{ color: t.textSub }}>{i18n.tasks?.priority || 'Приоритет'}</label>
                ...
                <div className="pc-form-field"><label style={{ color: t.textSub }}>{i18n.tasks?.folder || 'Папка'}</label>
              ...
              <div className="pc-modal-actions">
                <button type="button" className="pc-btn-ghost" style={{ color: t.textSub, borderColor: t.cardBorder }} onClick={() => setEditingTask(null)}>{i18n.common?.cancel || 'Отмена'}</button>
                <button type="submit" className="pc-btn-primary" style={{ background: '#fff', color: '#000' }}>{i18n.common?.save || 'Сохранить'}</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="pc-pages-inner" style={{ transform: `translateX(-${activePage * (100/8)}%)` }}>
              ...
              <header className="pc-header">
                ...
                <div className="pc-header-right">
                  ...
                  <button type="button" className="pc-add-btn" aria-label="Создать задание" style={{ background: t.text, color: '#000' }} onClick={() => setShowForm(true)}>
                    <span>+</span><span className="pc-add-btn-label">{i18n.tasks.addTask}</span>
                  </button>
                </div>
              </header>
                ...
                {showFolderForm ? (
                  <form onSubmit={createFolder} className="pc-folder-inline-form">
                    <input required value={folderForm.name} autoFocus onChange={e => setFolderForm({...folderForm, name: e.target.value})}
                      placeholder={i18n.folders?.folderName || "Папка..."} className="pc-input" style={{ background: t.inputBg, borderColor: t.cardBorder, color: t.text, height: 36, padding: '0 10px', fontSize: 13, width: 110 }} />
                  ...
                  <button type="button" className="pc-folder-tab" onClick={() => setShowFolderForm(true)}
                    ...
                    {i18n.folders?.newFolder ? `+ ${i18n.folders.newFolder}` : "+"}
                  </button>
                )}
              </div>
                ...
                <div className="pc-form-wrapper">
                  <form onSubmit={createTask} className="pc-form" style={{ background: 'rgba(20,20,20,0.95)', borderColor: t.cardBorderHover }}>
                    <div className="pc-form-header">
                      <span className="pc-form-label" style={{ color: t.textSub }}>{i18n.tasks?.newTask || "Новое задание"}</span>
                      <button type="button" className="pc-form-close" style={{ color: t.textMuted }} onClick={() => { setShowForm(false); setFormError('') }}>✕</button>
                    </div>
                    <input className="pc-input pc-input-title" style={{ background: t.inputBg, borderColor: t.cardBorder, color: t.text }} placeholder={i18n.tasks?.taskName || "Название задания..."} value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} autoFocus />
                    {formError && <div className="pc-form-error" style={{ background: `${t.danger}12`, borderColor: `${t.danger}33`, color:'#ff8899' }}>{formError}</div>}
                    <div className="pc-form-row">
                      <div className="pc-form-field"><label style={{ color: t.textSub }}>{i18n.tasks?.dueDate || 'Дата'}</label><input type="date" className="pc-input" style={{ background: t.inputBg, borderColor: t.cardBorder, color: t.text, colorScheme:'dark' }} value={formData.due_date} onChange={e => setFormData({...formData, due_date: e.target.value})} /></div>
                      <div className="pc-form-field"><label style={{ color: t.textSub }}>{i18n.tasks?.dueTime || 'Время'}</label><input type="time" className="pc-input" style={{ background: t.inputBg, borderColor: t.cardBorder, color: t.text, colorScheme:'dark' }} value={formData.due_time} onChange={e => setFormData({...formData, due_time: e.target.value})} /></div>
                      <div className="pc-form-field"><label style={{ color: t.textSub }}>{i18n.tasks?.priority || 'Приоритет'}</label>
                    ...
                    <div className="pc-form-footer">
                      <button type="button" className="pc-btn-ghost" style={{ color: t.textSub, borderColor: t.cardBorder }} onClick={() => { setShowForm(false); setFormError('') }}>{i18n.common?.cancel || 'Отмена'}</button>
              ...
              <div className="pc-task-list" ref={taskListRef}>
                ...
                : <>
                  ...
                  {doneTasks.length > 0 && <>
                    ...
                    {doneTasks.map((task, i) => <TaskCard key={task.id} task={task} t={t} index={i} onToggle={() => toggleTask(task)} onDelete={() => setDeleteConfirm(task)} lang={lang} i18n={i18n} onEdit={() => openEditTask(task)} folders={folders} completed={true} />)}
                  </>}
                </>}
              </div>
            </div>
            ...
            <div className="pc-page">
              ...
              <div style={{ flex:1, overflow:'auto' }}>
                <HabitTracker t={t} lang={lang} i18n={i18n} onHabitComplete={onHabitComplete} />
              </div>
            </div>
            ...
            <div className="pc-page">
              ...
              <div style={{ flex:1, overflow:'auto', padding: '16px 16px 100px' }}>
                <CharacterPanel xp={xp} completedCount={completedCount} theme={t} lang={lang} i18n={i18n} />
                ...
                <div style={{ marginTop: 20, display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                  {[
                    { label:lang === 'en' ? 'Total tasks' : 'Всего задач', value: totalCount },
                    { label:lang === 'en' ? 'Completed' : 'Выполнено', value: completedCount },
                    { label:lang === 'en' ? 'High prior.' : 'Высокий приор.', value: completedByPriority.high },
            ...
            <div className="pc-page">
              ...
              <div style={{ flex:1, overflowY:'auto' }}>
                ...
                <div style={{ padding:'0 16px 100px' }}>
                  <div style={{ fontSize:11, color: t.textSub, letterSpacing:'0.12em', fontFamily:'var(--font-mono)', marginBottom:14, paddingLeft:2, marginTop:24 }}>{lang === 'en' ? 'ANALYTICS' : 'АНАЛИТИКА'}</div>

                  {/* Progress bar by priority */}
                  <div style={{ background: t.card, border:`1px solid ${t.cardBorder}`, borderRadius:16, padding:'16px' }}>
                    <div style={{ fontSize:14, fontWeight:600, color: t.text, marginBottom:14 }}>{lang === 'en' ? 'Completed by priority' : 'Выполнено по приоритету'}</div>
                      ...
                      return (
                        <div key={key} style={{ marginBottom:12 }}>
                          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:5 }}>
                            <span style={{ fontSize:13, color: t.text }}>{lang === 'en' ? cfg.labelEn : cfg.labelRu}</span>
                  ...
                  <div style={{ background: t.card, border:`1px solid ${t.cardBorder}`, borderRadius:16, padding:'16px', marginTop:10 }}>
                    ...
                    {[
                      { label:lang === 'en' ? 'Tasks created' : 'Задач создано', value: totalCount, icon:'📋' },
                      { label: i18n.character?.completed || lang === 'en' ? 'Tasks done' : 'Задач выполнено', value: completedCount, icon:'✅' },
                      { label:lang === 'en' ? 'Folders created' : 'Папок создано', value: folders.length, icon:'📁' },
                      { label:lang === 'en' ? 'XP earned' : 'Опыт набрано', value: `${xp} XP`, icon:'⚡' },
                      { label:lang === 'en' ? 'Current rank' : 'Текущий ранг', value: `${rankInfo.rank} — ${rankInfo.label}`, icon:'🏅' },
                    ].map(s => (
            ...
            <div className="pc-page">
              <header className="pc-header">
                <div className="pc-header-left">
                  <span className="pc-header-folder-emoji">👤</span>
                  <span className="pc-header-folder-name" style={{ color: t.text }}>{i18n.profile.title}</span>
                </div>
              </header>
              <div style={{ flex:1, overflowY:'auto', padding:'16px 16px 100px',
                ...
                <div style={{ background: t.card, border:`1px solid ${t.cardBorder}`, borderRadius:20,
                  ...
                  <div style={{ fontSize:18, fontWeight:700, color: t.text }}>
                    {session?.user?.name || i18n.profile.user}
                  </div>
                  ...
                  <div style={{ display:'flex', gap:16, marginTop:16, justifyContent:'center' }}>
                    {[
                      { label: 'XP', value: xp },
                      { label: i18n.tasks.completed, value: completedCount },
                      { label: i18n.character.folders, value: folders.length },
                    ].map(s => (
                ...
                <div style={{ fontSize:11, color: t.textSub, letterSpacing:'0.12em',
                  fontFamily:'var(--font-mono)', marginBottom:10, paddingLeft:4 }}>{i18n.profile.notifications}</div>
                <div style={{ background: t.card, border:`1px solid ${t.cardBorder}`,
                  borderRadius:16, overflow:'hidden', marginBottom:16 }}>
                  {[
                    { icon:'🔔', label:i18n.profile.pushNotif, sub:i18n.profile.pushSub, toggle: true },
                    { icon:'✈️', label:i18n.profile.telegramBot, sub:'@sannder01_Bot', action: () =>
                ...
                {/* ── Внешний вид ── */}
                <div style={{ fontSize:11, color: t.textSub, letterSpacing:'0.12em',
                  fontFamily:'var(--font-mono)', marginBottom:10, paddingLeft:4 }}>{i18n.profile.appearance}</div>
                <div style={{ background: t.card, border:`1px solid ${t.cardBorder}`,
                  borderRadius:16, overflow:'hidden', marginBottom:16 }}>
                  {[
                ...
                {/* ── Аккаунт ── */}
                <div style={{ fontSize:11, color: t.textSub, letterSpacing:'0.12em',
                  fontFamily:'var(--font-mono)', marginBottom:10, paddingLeft:4 }}>{i18n.profile.account}</div>
                <div style={{ background: t.card, border:`1px solid ${t.cardBorder}`,
                  borderRadius:16, overflow:'hidden', marginBottom:16 }}>
                  {[
                    { icon:'G', label:'Google', sub:i18n.profile.googleConn, subColor: t.success },
                    { icon:'🍎', label:'Apple ID', sub:i18n.profile.appleReq, badge:i18n.profile.soon },
                  ].map((item, i, arr) => (
                    <div key={item.label} style={{ display:'flex', alignItems:'center', gap:12,
                      padding:'14px 16px',
                      borderBottom: i < arr.length-1 ? `1px solid ${t.cardBorder}` : 'none' }}>
                ...
                {/* ── О приложении ── */}
                <div style={{ fontSize:11, color: t.textSub, letterSpacing:'0.12em',
                  fontFamily:'var(--font-mono)', marginBottom:10, paddingLeft:4 }}>{i18n.profile.about}</div>
                <div style={{ background: t.card, border:`1px solid ${t.cardBorder}`,
                  borderRadius:16, overflow:'hidden', marginBottom:16 }}>
                  {[
                    { icon:'📋', label:i18n.profile.aboutApp, sub:i18n.profile.aboutSub },
                    { icon:'👤', label:i18n.profile.author, sub:'Sander Samarin' },
                    { icon:'⚡', label:i18n.profile.engine, sub:i18n.profile.engineSub },
                  ].map((item, i, arr) => (
                    <div key={item.label} style={{ display:'flex', alignItems:'center', gap:12,
                      padding:'12px 16px',
                      borderBottom: i < arr.length-1 ? `1px solid ${t.cardBorder}` : 'none' }}>
                ...
                {/* ── Язык ── */}
                <div style={{ fontSize:11, color: t.textSub, letterSpacing:'0.12em',
                  fontFamily:'var(--font-mono)', marginBottom:10, paddingLeft:4 }}>{i18n.profile.langSection}</div>
                <div style={{ background: t.card, border:`1px solid ${t.cardBorder}`,
                  borderRadius:16, overflow:'hidden', marginBottom:16 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:12, padding:'14px 16px' }}>
                    <span style={{ fontSize:18, width:28, textAlign:'center' }}>🌐</span>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:14, fontWeight:500, color: t.text }}>{i18n.profile.langLabel}</div>
                    </div>
                    <div style={{ display:'flex', gap:6 }}>
                      {['ru','en'].map(l => (
                        <button key={l} type="button" onClick={() => { setLang(l); if (typeof window !== 'undefined') localStorage.setItem('chronicle-lang', l) }}
                          style={{
                            padding:'6px 14px', borderRadius:10, cursor:'pointer',
                            fontFamily:'var(--font-mono)', fontSize:11, fontWeight:700,
                            border: lang === l ? '1px solid rgba(255,255,255,0.4)' : `1px solid ${t.cardBorder}`,
            ...
            {/* ── СТРАНИЦА 6: ЗАМЕТКИ ── */}
            <div className="pc-page">
              <NotesPage t={t} lang={lang} i18n={i18n} />
            </div>

            {/* ── СТРАНИЦА 7: ДОСТИЖЕНИЯ ── */}
            <div className="pc-page">
              <AchievementsPage t={t} lang={lang} i18n={i18n} />
            </div>
...
// ═══════════════════════════════════════════════════════════════════
function TaskCard({ task, t, index, onToggle, onDelete, onEdit, folders, completed, lang = 'ru', i18n = {} }) {
    ...
    <div className="pc-task task-card"
      ...
      <div className="pc-task-content" style={{ minWidth: 0 }}>
        ...
        <div className="pc-task-meta">
          <span className="pc-priority-badge" style={{ color: priority.color, background: `${priority.color}15`, borderColor: `${priority.color}28` }}>{lang === 'en' ? priority.labelEn : priority.labelRu}</span>
          {task.due_date && (
            <span className="pc-due-date" style={{ color: completed ? t.textMuted : getDaysColor(days, t) }}>
              ...
              {!completed && getDaysLeftLabel(days, lang, i18n) && <span className="pc-days-badge" style={{ color: getDaysColor(days,t), background: `${getDaysColor(days,t)}15`, borderColor: `${getDaysColor(days,t)}28` }}>{getDaysLeftLabel(days, lang, i18n)}</span>}
            </span>
          )}
...
function EmptyState({ t, activeFolder, setShowForm, lang = 'ru' }) {
  const msgs = {
    urgent: { icon:'◉', title:lang === 'en' ? 'No urgent tasks' : 'Срочных нет',  sub:lang === 'en' ? 'Nothing urgent' : 'Ничего не горит' },
    today:  { icon:'◎', title:lang === 'en' ? 'Free day' : 'День свободен', sub:lang === 'en' ? 'No tasks due today' : 'Нет задач с дедлайном сегодня' },
    all:    { icon:'◈', title:lang === 'en' ? 'Empty for now' : 'Пока пусто',    sub:lang === 'en' ? 'Create your first task' : 'Создай первое задание' },
  }
