...
export async function GET() {
  ...
  try {
    const r = await query('SELECT * FROM fitness_config WHERE user_id=$1', [session.user.id])
    return Response.json(r.rows[0] || { calorie_goal: 2000, target_weight: null })
  } catch (err) {
    console.error('[GET /api/fitness/config]', err)
    return Response.json({ calorie_goal: 2000, target_weight: null })
  }
}
...
export async function POST(req) {
  ...
  try {
    ...
    `, [session.user.id, calorie_goal || 2000, target_weight || null])
    return Response.json({ ok: true })
  } catch (err) {
    console.error('[POST /api/fitness/config]', err)
